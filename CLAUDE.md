...
- An experiment directory holds only `experiment.sqlite3`, the scratch directory `work/`, and the executables. The runner works inside `work/`, which it empties at the start of every generation, and calls `../evo`, `../evolve`, and `../initial-population`. They are **copies** of the build output, made on the experiment's first run, so a rebuild does not change an experiment that has started; to run new code, start a new experiment. The `provenance` table records what they were built from: the git revision, whether the checkout had uncommitted changes, and the external tools release.
