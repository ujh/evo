...
- `ruby/`, `runner`: tournament orchestration, and the benchmark (`ruby/benchmark.rb`, openings in `ruby/openings.rb`). `stats`, `ranking`: viewers. `multi`: rotates between several experiments.
...
- `Benchmark` (`ruby/benchmark.rb`) measures progress apart from the tournament. `RunGeneration#call` runs it after the tournament of every `keep_every`-th generation (the checkpoints, whose networks are kept); with `keep_every` 0 it never runs.
