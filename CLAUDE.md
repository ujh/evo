...
- Games run on a `WorkerPool` (`ruby/worker_pool.rb`): `concurrency` threads, created once per experiment, each running `gogui-twogtp` through `system`. On Ctrl-C the runner exits as soon as a running game returns and leaves the unfinished games in `data.json`, so resuming replays them.
