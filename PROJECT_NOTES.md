...
| --- | --- | --- |
| [`stats:65`](stats), [`ranking:37`](ranking) | Result lines are split on whitespace, so an empty column (GoGui leaves `RES_B` or `RES_W` empty when a program gives no score) shifts `RES_R` and `LEN`. Anything not starting with `B` counts as a White win. The runner was fixed on 24 Sep (see the decision log); the viewers still use the old parsing. | Reported win rates and game lengths can be wrong. Should reuse `ruby/game_result.rb`. |
...
- Ruby characterization tests (24 Sep): the runner has minitest coverage, run in CI through `mise run test`. Real GoGui result files showed two more cases the runner scores silently: a player that crashes mid-game still gets the referee's score for the position (`ERR` 1, `RES_R` `B+17.5` for the crashed Black), and a game stopped by the move limit is flagged `ERR` 1 but scored normally. A referee that fails to start leaves no `.dat` file, which raises `Errno::ENOENT`. The result-parsing fix should treat all three explicitly (done; see "Result parsing" below). A two-generation smoke run with 4 networks crashed while breeding generation 1: no network won a game, so the parent pool was empty. Because this happens in the smallest runs, the parent-pool fix is more urgent than the notes suggested. The owner wants CI split into one step per check; that is cleanup step 6.
- Result parsing (24 Sep): the runner reads results through `ruby/game_result.rb`, which splits on tabs and saves twogtp's stderr to `PREFIX.err`, because only stderr says which program died. Agreed rules: a referee win (`B+`/`W+`) counts, including games stopped by the move limit. A draw gives no points. A crashed network loses whatever the referee said. A crashed external bot, a missing referee score, any other GoGui error (for example an illegal move, which GoGui attributes to the program that rejected it), and a missing or empty `.dat` file give no points; the game is logged and recorded under `unscored` in `data.json`. Failed games are not retried, so broken setups stay visible.
